...
fn test_config() -> tigeropen::config::ClientConfig {
    tigeropen::config::ClientConfig {
        ...
        license: None,
        language: tigeropen::model::enums::Language::ZhCn,
        timezone: None,
...
fn test_quote_callback() {
    ...
    client.set_callbacks(Callbacks {
        on_quote: Some(Arc::new(move |data: pb::QuoteData| {
            ...
            assert_eq!(data.latest_price, Some(155.0));
            count_clone.fetch_add(1, Ordering::SeqCst);
        })),
        ..Default::default()
    });
...
fn test_asset_callback() {
    ...
    client.set_callbacks(Callbacks {
        on_asset: Some(Arc::new(move |data: pb::AssetData| {
            ...
            assert_eq!(data.net_liquidation, 100000.5);
            count_clone.fetch_add(1, Ordering::SeqCst);
        })),
        ..Default::default()
    });
...
fn test_tick_callback() {
    ...
    let response = pb::Response {
        ...
        msg: None,
        body: Some(pb::PushData {
            data_type: DataType::TradeTick as i32,
            body: Some(pb::push_data::Body::TradeTickData(pb::TradeTickData {
                symbol: "TSLA".into(),
                ..Default::default()
            })),
        }),
...
fn test_full_tick_callback() {
    ...
    let response = pb::Response {
        ...
        body: Some(pb::PushData {
            data_type: DataType::TradeTick as i32,
            body: Some(pb::push_data::Body::TickData(pb::TickData {
                symbol: "AAPL".into(),
                ..Default::default()
            })),
        }),
...
fn test_option_top_callback() {
    ...
    let count_clone = count.clone();
    client.set_callbacks(Callbacks {
        on_option_top: Some(Arc::new(move |data: pb::OptionTopData| {
            assert_eq!(data.market, "US");
            count_clone.fetch_add(1, Ordering::SeqCst);
        })),
        ..Default::default()
    ...
    let response = pb::Response {
        ...
        msg: None,
        body: Some(pb::PushData {
            data_type: DataType::OptionTop as i32,
            body: Some(pb::push_data::Body::OptionTopData(pb::OptionTopData {
                market: "US".into(),
                timestamp: 1700000000,
                top_data: vec![],
            })),
...
fn test_future_callback() {
    ...
    let count_clone = count.clone();
    client.set_callbacks(Callbacks {
        on_future: Some(Arc::new(move |data: pb::QuoteData| {
            assert_eq!(data.symbol, "ESZ3");
            count_clone.fetch_add(1, Ordering::SeqCst);
        })),
        ..Default::default()
    });
    ...
    let response = pb::Response {
        ...
        msg: None,
        body: Some(pb::PushData {
            data_type: DataType::Future as i32,
            body: Some(pb::push_data::Body::QuoteData(pb::QuoteData {
                symbol: "ESZ3".into(),
                ..Default::default()
            })),
